...
contract LaunchUpdateParticipationTest is Test, Launch, LaunchTestBase {
    ...
    function test_UserTokenTrackingIssue() public {
        ...
        console.log("user1 balance", currency.balanceOf(user1));
        ...
        uint256 updatedUserTokens = launch.getUserTokensByLaunchGroup(testLaunchGroupId, testUserId);
        console.log("after user balance", currency.balanceOf(user1));
        assertNotEq(
            updatedUserTokens,
            initialUserTokens - updateRequest.tokenAmount,
            "User tokens should decrease by token amount, not currency amount"
        );
    }
